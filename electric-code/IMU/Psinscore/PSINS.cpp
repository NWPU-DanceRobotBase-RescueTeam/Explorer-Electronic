...
{
	CVect pp = v;
	double *p, *pEnd=&pp.dd[pp.row*pp.clm];
	for(int i=1; i<k; i++)
	{
		double *p=pp.dd;
...
void CSINSTDKF::TDUpdate(const CVect3 *wm, const CVect3 *vm, int nSamples, double ts, int nStep)
{
	sins.Update(wm, vm, nSamples, ts);
	Feedback(sins.nts);
